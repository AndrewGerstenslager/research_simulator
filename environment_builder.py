    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        for wall in walls:
            if wall.rect.collidepoint(event.pos):
                wall.selected = True
                selected_wall = wall
                for handle, direction in zip(wall.get_handles(), ['top-left', 'top-right', 'bottom-left', 'bottom-right', 'top-center', 'bottom-center', 'left-center', 'right-center']):
        ...
        elif selected_wall and event.buttons[0]:  # Check if the left mouse button is held down
            is_dragging = True
...
def handle_keyboard_events(event):
    ...
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_c and selected_wall:
            copied_wall = Wall(selected_wall.rect.x, selected_wall.rect.y, selected_wall.rect.width, selected_wall.rect.height)
...
buttons = [
    ...
    Button(850, 170, 100, 50, 'Paste Wall', paste_wall, color=DISABLED_GRAY if not copied_wall else BLACK),
...
while running:
    ...
    pygame.draw.rect(screen, GRAY, (LEFT_BOUNDARY, TOP_BOUNDARY, RIGHT_BOUNDARY - LEFT_BOUNDARY, BOTTOM_BOUNDARY - TOP_BOUNDARY))
    ...
    # Draw the buttons
    for button in buttons:
        button.color = DISABLED_GRAY if button.text == 'Paste Wall' and not copied_wall else BLACK
        button.draw(screen)
